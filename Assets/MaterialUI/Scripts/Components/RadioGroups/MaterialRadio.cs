...
namespace MaterialUI
{
    ...
    public class MaterialRadio : ToggleBase
    {
        ...
        public override void TurnOnInstant()
        {
			...
			if (m_DotImage.gameObject.tag == "Big") {
            m_DotImage.rectTransform.sizeDelta = new Vector2(50, 50);
			} else {
        ...
        public override void AnimOn()
        {
			...
			float tempSize = 24;
			if (m_DotImage.gameObject.tag == "Big") {
				tempSize = Tween.QuintOut(m_CurrentDotSize, 50, m_AnimDeltaTime, m_AnimationDuration);
			} else {
				 tempSize = Tween.QuintOut(m_CurrentDotSize, 24, m_AnimDeltaTime, m_AnimationDuration);
